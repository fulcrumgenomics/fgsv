...
import com.fulcrumgenomics.FgBioDef.{FgBioEnum, FilePath}
...
(@arg(flag='i', doc="The input text file of pileups generated by SvPileup") input: FilePath,
 ...
 @arg(flag='d', doc="Distance threshold below which to cluster breakpoints") maxDist: Int = 100,
) extends SvTool with AggregateSvPileupDef {
  ...
  override def execute(): Unit = {
    ...
    val pileupsByContigStrand: Map[BreakpointContigOrientation, PileupGroup] =
      pileups.groupMap(BreakpointContigOrientation(_))(identity)

    // Within each contig+strand group, identify all immediate "neighbors" of each pileup (other pileups within the
    // distance threshold)
    val pileupToNeighbors: Map[BreakpointContigOrientation, Map[BreakpointPileup, PileupGroup]] =
      pileupsByContigStrand.view.mapValues(AggregateSvPileup.pileupToNeighbors(_, maxDist)).toMap

    // Within each contig+strand group, recursively build (transitive) clusters of neighboring pileups
    val pileupClusters: Map[BreakpointContigOrientation, Seq[PileupGroup]] =
      pileupToNeighbors.view.mapValues(AggregateSvPileup.toClusters).toMap

    // Aggregate each cluster
    val aggregatedPileups: Map[BreakpointContigOrientation, Seq[AggregatedBreakpointPileup]] =
      pileupClusters.view.mapValues(_.map(AggregatedBreakpointPileup.apply)).toMap

    // Write output file
    Metric.write[AggregatedBreakpointPileup](output, aggregatedPileups.values.flatten)
  }

}


object AggregateSvPileup extends AggregateSvPileupDef {
  ...
  private def pileupToNeighbors(pileups: PileupGroup, maxDist: Int): Map[BreakpointPileup, PileupGroup] = {
    val map: mutable.Map[BreakpointPileup, PileupGroup] = mutable.Map.from(pileups.map((_, Seq())))
    for (pileup1 <- pileups; pileup2 <- pileups) {
      if (pileup1.hashCode() < pileup2.hashCode() && isCluster(pileup1, pileup2, maxDist)) {
        map.put(pileup1, map(pileup1).appended(pileup2))
        map.put(pileup2, map(pileup2).appended(pileup1))
      }
    }
    map.toMap
  }

  /** Returns true iff the two breakpoints can be clustered, i.e., they have the same left and right chromosomes and
   * strand, and the distance between their left and right coordinates is below the threshold (inclusive) */
  ...
  def toClusters(pileupToNeighbors: Map[BreakpointPileup, PileupGroup]): Seq[PileupGroup] = {
    finalizeClusters(Seq(), pileupToNeighbors)
  }

  /**
   * Similar to `toClusters()`, but picking up from a partially complete recursive call stack.
   * @param existingClusters The set of clusters that have already been assembled
   * @param remainingNeighborGroups The set of remaining neighbor groups, which are all separate from the existing
   *                                clusters (don't belong with any of the existing clusters)
   * @return The set of pileup clusters
   */
  @tailrec
  private def finalizeClusters(existingClusters: Seq[PileupGroup],
                               remainingNeighborGroups: Map[BreakpointPileup, PileupGroup]): Seq[PileupGroup] = {
    if (remainingNeighborGroups.isEmpty) {
      existingClusters
    } else {
      extractOneCluster(remainingNeighborGroups) match {
        case (pileupGroup: PileupGroup, remaining: Map[BreakpointPileup, PileupGroup]) =>
          finalizeClusters(existingClusters :+ pileupGroup, remaining)
      }
    }
  ...
  /**
   * Extracts the complete cluster containing the given pileup from the map of pileups to their neighbor sets, and
   * returns the cluster along with the reduced map of pileup to neighbor set such that no remaining pileups in the map
   * belong in a cluster with the returned cluster.
   * @param pileup The pileup representative of the cluster that will be returned
   * @param pileupToNeighbors Map of pileup to its set of neighbors
   * @return (1) The complete cluster for the given pileup; (2) Remaining map of pileup to neighbor set, all of which
   *         are independent of the returned cluster.
   */
  def extractClusterFor(pileup: BreakpointPileup, pileupToNeighbors: Map[BreakpointPileup, PileupGroup]):
  (PileupGroup, Map[BreakpointPileup, PileupGroup]) = {
    finalizeClusterFor(pileup, Seq(), pileupToNeighbors)
  }

  /**
   * Similar to `extractClusterFor()`, but picking up from a partially complete recursive call stack.
   * Implementation note: the implementation is not tail-recursive, but in practice the recursion depth is small.
   * @param pileup The pileup representative of the cluster that will be returned
   * @param existingCluster Partially complete cluster that the given pileup belongs to
   * @param remainingNeighborGroups Additional pileup-to-neighbor-set relationships to examine and possibly add to the
   *                                growing cluster
   * @return (1) The complete cluster for the given pileup; (2) Remaining map of pileup to neighbor set
   */
  private def finalizeClusterFor(
                          pileup: BreakpointPileup,
                          existingCluster: PileupGroup,
                          remainingNeighborGroups: Map[BreakpointPileup, PileupGroup]):
  (PileupGroup, Map[BreakpointPileup, PileupGroup]) = {
    if (existingCluster.contains(pileup) && !remainingNeighborGroups.contains(pileup)) {
      (existingCluster, remainingNeighborGroups)
    } else {
      val neighbors: PileupGroup = remainingNeighborGroups(pileup)
      val reducedRemainingGroups: Map[BreakpointPileup, PileupGroup] = remainingNeighborGroups.removed(pileup)
      if (reducedRemainingGroups.isEmpty) (neighbors :+ pileup, Map())
      else neighbors.foldLeft[(PileupGroup, Map[BreakpointPileup, PileupGroup])](
        ...
        (currClusterAndRemaining: (PileupGroup, Map[BreakpointPileup, PileupGroup]), nextNeighbor: BreakpointPileup) => {
          currClusterAndRemaining match {
            case (currCluster, remaining) =>
              finalizeClusterFor(nextNeighbor, currCluster, remaining) match {
...
object BreakpointCategory extends Enumeration {
  ...
  val POSSIBLE_DELETION = "Possible deletion"
  val POSSIBLE_INTRA_CONTIG = "Possible intra-contig rearrangement"
  val POSSIBLE_INTER_CONTIG = "Possible inter-contig rearrangement"
  ...
  def apply(breakpointPileup: BreakpointPileup): BreakpointCategory = {
    if (breakpointPileup.left_contig != breakpointPileup.right_contig) {
      POSSIBLE_INTER_CONTIG
    } else {
      if (breakpointPileup.left_strand != breakpointPileup.right_strand) {
        POSSIBLE_INTRA_CONTIG
      } else POSSIBLE_DELETION
    }
  }
}
...
object AggregatedBreakpointPileup extends AggregateSvPileupDef {
